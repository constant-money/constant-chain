...
func (blockService BlockService) GetPortalShieldingRequestStatus(reqTxID string) (*metadata2.PortalShieldingRequestStatus, error) {
	stateDB := blockService.BlockChain.GetBeaconBestState().GetBeaconFeatureStateDB()
	...
	var status metadata2.PortalShieldingRequestStatus
	err = json.Unmarshal(data, &status)
	if err != nil {
		return nil, err
	}
