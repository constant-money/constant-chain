...
// func (c CoinV1) Decrypt(keySet IKeySet) (PlainCoin, error) {
// 	if keySet == nil {
...
// 	}
// 	if len(keySet.GetReadOnlyKey().Rk) == 0 && len(keySet.GetPrivateKey()) == 0 {
// 		err := errors.New("Cannot Decrypt CoinV1: Keyset does not contain viewkey or privatekey")
// 		return nil, errhandler.NewPrivacyErr(errhandler.DecryptOutputCoinErr, err)
// 	}

// 	if bytes.Equal(c.GetPublicKey().ToBytesS(), keySet.GetPaymentAddress().Pk[:]) {
// 		result := &CoinV1{
...
// 			if len(keySet.GetReadOnlyKey().Rk) > 0 {
// 				msg, err := henc.HybridDecrypt(c.CoinDetailsEncrypted, new(operation.Scalar).FromBytesS(keySet.GetReadOnlyKey().Rk))
// 				if err != nil {
// 					return nil, errhandler.NewPrivacyErr(errhandler.DecryptOutputCoinErr, err)
// 				}
...
// 				new(operation.Scalar).FromBytesS(keySet.GetPrivateKey()),
